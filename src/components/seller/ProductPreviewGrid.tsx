...
export default function ProductPreviewGrid({ products, onRemoveItem }: ProductPreviewGridProps) {
  return (
    /* 🎯 OPTIMIERT: Abstand nach oben verringert (falls übergeordnetes Flex aktiv ist) und Box kompakter gestaltet */
    <div className="border border-black p-5 bg-zinc-50 flex flex-col gap-3 animate-in fade-in duration-200 w-full -mt-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-1">
        {products.map((prod, idx) => {
          ...
          return (
            ...
            >
              {/* 🎯 OPTIMIERT: Von breiter Box zu perfekt quadratischem, kleinen X-Button umgebaut */}
              <button
                onClick={() => onRemoveItem(idx)}
                className="absolute top-2 right-2 z-10 w-5 h-5 flex items-center justify-center bg-white border border-zinc-200 text-zinc-400 hover:text-red-600 hover:border-red-600 text-[10px] font-sans cursor-pointer transition-colors shadow-xs"
