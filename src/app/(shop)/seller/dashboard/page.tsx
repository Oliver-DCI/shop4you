...
export default function SellerDashboardPage() {
  ...
  // 📝 ANGEPASSTES TEMPLATE MIT SYSTEM-KONFORMEN KATEGORIEN
  const insertTemplate = () => {
    const template = [
      { 
        "title": "UltraBook Pro 14", 
        "price": 999.00, 
        "category": "Notebooks", // Erlaubt!
        "brand": "S4Y",
  ...
  if (loading) {
    return <div className="p-20 text-center font-mono text-xs uppercase tracking-widest text-zinc-400 bg-white min-h-screen">Synchronisiere Live-Datenbank...</div>;
  }
      ...
      <div className="max-w-[1400px] mx-auto flex flex-col gap-8 relative z-10">
          ...
          <div className="lg:col-span-2 bg-white border border-zinc-200 p-6 flex flex-col gap-4">
            ...
            <div className="h-48 w-full flex items-end gap-2 pt-6 relative border-b border-zinc-200 bg-zinc-50 px-2">
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-30 px-2 py-4">
                <div className="w-full border-t border-dashed border-zinc-300 text-[8px] font-mono text-zinc-400 pt-0.5">MAX PROPORTIONAL</div>
                <div className="w-full border-t border-dashed border-zinc-300 text-[8px] font-mono text-zinc-400 pt-0.5">MEDIAN SCALE</div>
              </div>
                ...
                >
                  <div className="absolute top-2 left-0 right-0 text-center">
                    <span 
                      ...
                      className="text-[8px] font-mono text-zinc-400 hover:text-black uppercase tracking-widest transition-colors cursor-pointer select-none"
                    >
                      [ Artikel löschen ]
                    </span>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-baseline">
                      ...
                      {product.brand && <span className="text-[8px] font-mono text-zinc-500 font-bold uppercase">{product.brand}</span>}
                    </div>
        ...
        {livePreview.length > 0 && (
          <div className="border border-zinc-200 p-6 bg-zinc-50/50 flex flex-col gap-4">
            <h3 className="text-[10px] font-medium uppercase tracking-widest text-zinc-400 font-mono">[ Live-Parsing Vorschau ]</h3>
                ...
                return (
                  <div key={idx} className={`border p-4 pt-10 relative overflow-hidden group bg-white ${isInvalid ? 'border-red-400 bg-red-50/20' : 'border-zinc-200'}`}>
                    <div className="absolute top-0 left-0 right-0 text-center mb-8">
                      <span onClick={(e) => removeProductFromPreview(e, idx)} className="text-[9px] font-mono text-zinc-400 hover:text-black uppercase tracking-widest transition-colors cursor-pointer inline-block mt-2">[ Vorschau entfernen ]</span>
                    </div>
                    <div>
                      <div className="flex justify-between items-baseline">
                        <span className={`text-[8px] font-mono uppercase tracking-widest block mb-1 ${isInvalid ? 'text-red-600 font-bold' : 'text-zinc-400'}`}>
                          {prod.category || 'Keine'} {isInvalid && '⚠️'}
                        </span>
                        {prod.brand && <span className="text-[8px] font-mono text-zinc-500 uppercase font-bold">{prod.brand}</span>}
                      </div>
